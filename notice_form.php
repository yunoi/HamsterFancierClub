...
<?php
  if($mode === "insert"){
    echo ("<h3 id='board_title'>공지사항 > 글쓰기</h3>
      <form name='board_form' action='notice_crud.php?mode=$mode' method='post' enctype='multipart/form-data'>
    ");

  } else {
    echo ("<h3 id='board_title'>공지사항 > 글수정</h3>
      <form name='board_form' action='notice_crud.php?mode=$mode&num=$num&page=$page' method='post' enctype='multipart/form-data'>
    ");
  }
?>
          <ul id="board_form">
          ...
          <ul class="buttons">
            ...
            <li><button type="button" name="button" onclick="location.href='notice_list.php'">목록</button></li>
          </ul>
        </form>
      </div>
    </section>
